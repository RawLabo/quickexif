...
fn parse_rw2() -> LogResult<()> {
    ...
    info!("{:?}", result.get(panasonic_tags::black_level_b).map(|x| x.u16()));
    info!("{:?}", result.get(panasonic_tags::white_balance_r).map(|x| x.u16()));
    info!("{:?}", result.get(panasonic_tags::white_balance_g).map(|x| x.u16()));
    info!("{:?}", result.get(panasonic_tags::white_balance_b).map(|x| x.u16()));
    info!("{:?}", result.get(panasonic_tags::strip).map(|x| x.u32()));
    info!("{:?}", result.get(panasonic_tags::strip_len).map(|x| x.u32()));
    info!("{:?}", result.get(panasonic_tags::crop_top).map(|x| x.u16()));
    info!("{:?}", result.get(panasonic_tags::crop_left).map(|x| x.u16()));
    info!("{:?}", result.get(panasonic_tags::crop_bottom).map(|x| x.u16()));
    info!("{:?}", result.get(panasonic_tags::crop_right).map(|x| x.u16()));
